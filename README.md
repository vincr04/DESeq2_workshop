...
Bioconductor also contains some 'pre-packaged' datasets that can be easily downloaded and used as examples. Today, we will work on the 'pasilla' dataset ([Brooks et al., Genome Research 2011](https://pubmed.ncbi.nlm.nih.gov/20921232/)) which explores the effects of RNAi knockdown of *pasilla*, a nuclear RNA binding protein implicated in splicing and ortholog of mammalian NOVA1 and NOVA2, on the transcriptome on *Drosophila melanogaster* cell cultures.

As above, use Google to find out how to install the 'pasilla' dataset, and don't forget to type `library("pasilla")` to load the package and the data. 
...
```
and arrange the order of the columns in the read **counts matrix**
```
...
Read counts from the 'pasilla' dataset first need to be stored into a *DESeqDataSet*, which we will call `dds`. 
